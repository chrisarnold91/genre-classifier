...
GENRES = ['classical', 'rock', 'jazz', 'country']
...
def main():
    if len(argv) != 2 or int(argv[1]) not in range(HASH_FUNCTIONS):
        print "usage: pitches.py <int in range(" + str(HASH_FUNCTIONS) + ")>"
        return
    ...
    training_hashes = {}
    for genre in GENRES:
        build_hash_table('midi-' + genre, training_hashes, genre,
            hash_function[argv[1]])

    sample_hashes = {}
    build_hash_table('test-set', sample_hashes, 'sample',
        hash_function[argv[1]])

    buckets, genres = match(training_hashes, sample_hashes)
    plot_graph(buckets)

    print "Is this track " + max(genres) + "?"

def build_hash_table(directory, hashes, genre, hash_function):
    """
        ...
        for sample_time in value:
            for training_time in training[key]:
                ...
                if key in training:
                    ...
                    if training_time.genre not in genres:
                        genres[training_time.genre] = 1
                    else:
                        genres[training_time.genre] += 1
